...
local function check_one_node(node, lvl)
    ...
    if not (not node.parent or node.parent[node:idx()] == node) then
        error("node:idx = "..tostring(node:idx() or nil)..", parent[idx] = "..tostring(node.parent[node:idx()])..
        ", node = "..tostring(node))
    end
    local cur = node.firstchild
    assert(cur == node[1], "#node = "..#node..", cur = "..tostring(cur)..", node[1] = "..tostring(node[1]))
    local count = 0
    for i, v in ipairs(node) do
        assert(node[v] == i, "node[v] = "..tostring(node[v])..", i = "..i)
        assert(v:idx() == i, "v:idx = "..v:idx()..", i = "..i)
        assert(v.parent == node, "v.parent = "..tostring(v.parent)..", node = "..tostring(node))
